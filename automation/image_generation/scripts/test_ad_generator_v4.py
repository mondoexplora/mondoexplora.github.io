#!/usr/bin/env python3
"""
Instagram Story Ad Generator v4
Refined version with proper Instagram Story spacing and improved typography
"""
...
def draw_strikethrough_text(draw, x, y, text, font, fill_color):
    ...
    draw.line([(x, line_y), (x + text_width, line_y)], fill=fill_color, width=2)
    ...
    try:
        ...
        if download_image(hotel_data['hero_image'], temp_img_path):
            ...
            if hotel_data.get('value') and hotel_data['value'] > hotel_data['price']:
                ...
                # Use smaller fonts (30% smaller than font_large)
                price_font_size = int(font_large.size * 0.7)  # 30% smaller
                try:
                ...
                box_draw.text((current_x, current_y), current_price, price_font, fill=(34, 197, 94))  # Green color
...
def main():
    print("Creating Instagram Story Ad v4...")
    ...
    # Load refined fonts (50% bigger than before)
    try:
        # Try to use a more refined font (50% bigger)
        font_extra_bold = ImageFont.truetype("/System/Library/Fonts/Helvetica.ttc", 54)  # 50% bigger than 36
        font_bold = ImageFont.truetype("/System/Library/Fonts/Helvetica.ttc", 54)  # Same size as extra bold
        font_medium = ImageFont.truetype("/System/Library/Fonts/Helvetica.ttc", 32)  # Bigger for vendor names
    ...
    # Calculate hotel box dimensions (smaller)
    hotel_box_margin = 15
    ...
    filename = f"{output_dir}/instagram_story_v4_{timestamp}.jpg"
    ...
    print(f"✅ Instagram Story ad v4 saved: {filename}")
